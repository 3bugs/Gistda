...
exports['default'] = function (_ref) {
  ...
  try {
    for (var _iterator = data.entries()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      const startAngle = scale(t);
      const endAngle = scale(t + value);

      console.log('@@@@@');
      console.log(`@@@@@ SECTOR - Start angle: ${(2 * Math.PI) - (endAngle + (Math.PI / 2))}, End angle: ${(2 * Math.PI) - (startAngle + (Math.PI / 2))}`);
      ...
      curves.push((0, _ops.enhance)(compute, {
        ...
        sector: (0, _sector2['default'])({
          ...
          r: r,
          R: R,
          end: (3 * Math.PI) - (startAngle + (Math.PI / 2)),
          start: (3 * Math.PI) - (endAngle + (Math.PI / 2)),
        })
      }));
      t += value;
    }
